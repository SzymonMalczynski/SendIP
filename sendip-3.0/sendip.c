...
static void print_usage(void) {
	sendip_module *mod;
	int i;
	printf("Usage: %s [-v] [-d data] [-h] [-f datafile] -x xmlfile hostname\n         OR\n",progname);
	printf("       %s [-v] [-d data] [-h] [-f datafile] -p module [-p module...] [module options] hostname\n",progname);

	printf(" -x xmlfile\tused to specify all the packet headers using XML.  XML files contain:\n");
	printf("\t\tA single <packet> tag containing:\n");
	printf("\t\tMany <header type=\"module\"> tag which load modules (like -p option)\n");
	printf("\t\tOptionally a <data>data</data> tag which specifies data as for the -d option\n");
	printf("\t\t<header> tags may contain any number of <option name=\"name\" value=\"val\"/> tags\n");
	printf("\t\t\tThe name is the XML option name as listed below\n\n");
	...
	printf(" -v\t\tbe verbose (dump packet data before sending)\n");

	printf("\n\nModules are loaded in the order the -p option or <header> tag appears.\n");
	printf(" The headers from each module are put immediately inside the headers from the\n");
	printf(" previos model in the final packet.  For example, to embed bgp inside tcp inside\n");
	printf(" ipv4, do\n");
	printf("sendip -p ipv4 -p tcp -p bgp ....\n\n");

	printf("Modules available at compile time:\n");
...
int main(int argc, char *const argv[]) {
	...
	bool usage=FALSE, verbosity=FALSE, have_xml=FALSE;
	int firstxmlopt=argc;
	...
	while(gnuoptind<argc && (EOF != (optc=gnugetopt(argc,argv,"-p:vd:hf:x:")))) {
		switch(optc) {
		case 'x':
			if(have_xml) {
				fprintf(stderr,"Can't specify -x more than once\n");
				usage=TRUE;
			} else if(num_modules) {
				fprintf(stderr,"Can't specify -p and -x\n");
				usage=TRUE;
			} else {
				char **extra_argv=NULL;
				int extra_argc=parse_xml(gnuoptarg,&extra_argv);
				if(extra_argc <= 0) {
					usage=TRUE;
					fprintf(stderr,"Error parsing XML file %s\n",gnuoptarg);
				} else {
					// copy old args and add new ones
					char **new_argv = (char **)malloc((argc+extra_argc)*sizeof(char *));
					if(new_argv==NULL) {
						fprintf(stderr,"OUT OF MEMOERY\n");
						exit(1);
					}
					memcpy(new_argv,argv,argc*sizeof(char *));
					memcpy(new_argv+argc,extra_argv,extra_argc*sizeof(char *));
					argc+=extra_argc;
					argv=new_argv;
					have_xml=TRUE;
				}
			...
			break;
		case 'p':
			if(have_xml && (gnuoptind <= firstxmlopt)) {
				fprintf(stderr,"Can't specify -x and -p\n");
				usage=TRUE;
			} else {
		...
		case ':':
			if(have_xml && (gnuoptind <= firstxmlopt)) {
				fprintf(stderr,"Can't specify other options with -x\n");
				usage = TRUE;
			}
	...
	gnuopterr=1;
	gnuoptind=0;
	while(EOF != (optc=getopt_long_only(argc,argv,"p:vd:hf:x:X:",opts,&longindex))) {
		...
		case '?':
			...
			break;
		case 'X':
			// XML long format args
			{
				char *modn = gnuoptarg;
				char *opt = modn;
				char *val = opt;
				
 printf("GOT XML OPTION %s\n",gnuoptarg);

				while(*opt && *opt != '=') opt++;
				if(!*opt) {
					usage=TRUE;
					fprintf(stderr,"Invalid format -X option %s\n",gnuoptarg);
					break;
				}
				*(opt++)='\0';
				val=opt;
				...
				*(val++)='\0';
				if(!strcmp(val,"r") || !strcmp(val,"random")) {
...
printf(" MOD %s OPT %s VAL %s\n",modn,opt,val);

				for(mod=first;mod!=NULL;mod=mod->next) {
					if(!strcmp(mod->name,modn)) {
						int i;
printf(" MATCHED MODULE\n");
						for(i=0;i<mod->num_opts;i++) {
							if(!strcmp(mod->opts[i].xmlname,opt)) {
								opt = malloc(strlen(mod->opts[i].optname)+1);
								sprintf(opt,"%c%s",mod->optchar,mod->opts[i].optname);
								//								opt = strdup(mod->opts[i].optname);
printf(" OPTION IS %s\n",opt);
								break;
							}
						}
						if(!mod->do_opt(opt,val,mod->pack)) {
							usage=TRUE;
						}
					}
				}
